...
import threading
DATA_HANDLE = '/val'
MOTION_EMB_PATH = '../outs/embeds' + DATA_HANDLE
LOSS_PATH = '../outs/losses' + DATA_HANDLE
...
class VADHead(DETRHead):
    ...
    def forward(
        ...
        if self.motion_decoder is not None:
            ...
            motion_hs_only = motion_hs.clone()
            motion_hs = torch.cat(
                [motion_hs_only, ca_motion_query], dim=-1
            )  # [B, A, fut_mode, 2D]
        else:
            raise NotImplementedError("Not implement yet")
        ...
        paths = [MOTION_EMB_PATH + '/' + piece + '_' + affix for piece in ['motion','map_motion','ego_agent','ego_map']]
        bits = [motion_hs_only.detach().clone(), ca_motion_query.detach().clone(), ego_agent_query.permute(1, 0, 2).detach().clone(), ego_map_query.permute(1, 0, 2).detach().clone()]
        print(dict(zip(paths, bits)))
        thread = threading.Thread(target=save_tensor, args = [dict(zip(paths, bits))])
        thread.start()
        outs = {
            "bev_embed": bev_embed,
    ...
    def loss_planning(
        ...
        Args:
            ego_fut_preds (Tensor): [B, ego_fut_mode, fut_ts, 2]                out direct from dict 'same key'
            ego_fut_gt (Tensor): [B, fut_ts, 2]                                  in 'ego_his_trajs'
            ego_fut_masks (Tensor): [B, fut_ts]                                  in 'ego_fut_masks'
            ego_fut_cmd (Tensor): [B, ego_fut_mode]                              in 'ego_fut_cmd'
            lane_preds (Tensor): [B, num_vec, num_pts, 2]                       out 'map_all_pts_preds'[-1]
            lane_score_preds (Tensor): [B, num_vec, 3]                          out 'map_all_cls_scores'[-1].sigmoid()
            agent_preds (Tensor): [B, num_agent, 2]                             out 'all_bbox_preds'[-1][...,0:2]
            agent_fut_preds (Tensor): [B, num_agent, fut_mode, fut_ts, 2]       out 'all_traj_preds'[-1].view(batch, num_agent, self.fut_mode, self.fut_ts, 2)
            agent_score_preds (Tensor): [B, num_agent, 10]                      out 'all_cls_scores'[-1].sigmoid()
            agent_fut_cls_scores (Tensor): [B, num_agent, fut_mode]             out 'all_traj_cls_scores'[-1].view(batch, num_agent, self.fut_mode)
        Returns:
        ...
        ### MARK
        loss_paths = [LOSS_PATH + '/' + piece + '_' + affix for piece in ["loss_plan_reg","loss_plan_bound","loss_plan_col","loss_plan_dir"]]
        result_list = [loss_plan_l1,loss_plan_bound,loss_plan_col,loss_plan_dir]
        thread = threading.Thread(target=save_tensor, args = [dict(zip(loss_paths,result_list))])
        thread.start()
