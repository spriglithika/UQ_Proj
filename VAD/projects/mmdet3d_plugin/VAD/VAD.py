...
class VAD(MVXTwoStageDetector):
    ...
    def forward_pts_train(
        ...
        affix = affix
        if not affix:
            affix = f"{torch.distributed.get_rank()}_{self.counter}.pth"
        ...
        losses = self.pts_bbox_head.loss(*loss_inputs, img_metas=img_metas,affix=affix)
        self.counter += 1
        return losses
